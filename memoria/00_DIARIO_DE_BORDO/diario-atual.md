...
- Nova tentativa: a reexecução do job sobre o SHA cancelado foi rejeitada pelo Pages porque reutilizou o mesmo identificador de deployment. Um novo SHA disparou execução limpa e única sob a configuração corrigida, mas o serviço permaneceu em `deployment_queued` até atingir novamente o timeout padrão de 10 minutos.
...
- Ajuste do checkout: a primeira tentativa de criar `gh-pages` encerrou antes do push porque `actions/checkout@v6` armazena a credencial em arquivo temporário, não no `extraheader` local esperado. O workflow foi ajustado para usar `${{ github.token }}` somente no comando de push; o valor permanece mascarado e não integra o commit publicado.
- Reinicialização controlada: após nova falha do serviço com build aprovado, o site foi despublicado e reativado sobre `gh-pages /(root)` para limpar o estado anterior. A execução seguinte avançou de `deployment_queued` para `deployment_in_progress`, mas o pipeline automático de branch cancelou o deployment ao atingir seu próprio limite de 10 minutos.
- Configuração final: a origem retornou a `GitHub Actions` e o workflow voltou a enviar o artefato seguro pela API oficial sem cancelamento prematuro. Essa combinação preserva a reinicialização realizada e permite que o estado `deployment_in_progress` conclua além de 10 minutos.
- Resultado final observado: o run `31109845726` aprovou build e upload, manteve o deployment ativo por 60 minutos sem cancelá-lo e encerrou por expiração da janela de monitoramento. O backend não retornou sucesso nem falha terminal nesse período; o endereço público permaneceu em HTTP `404` após a reinicialização.
- Estado de entrega: `main` e `origin/main` contêm os dados publicados mais recentes, o artefato seguro e a correção do workflow. A ativação online permanece bloqueada no serviço GitHub Pages, apesar de o painel público de status indicar Pages operacional. O deployment foi deixado ativo para eventual conclusão pelo serviço; não foi iniciado novo retry concorrente.
- Controle de repetição: commits exclusivamente documentais podem usar `[skip pages]` para não iniciar nova publicação. A próxima tentativa deve ocorrer somente após mudança de estado do serviço ou orientação do suporte GitHub, evitando acumular deployments concorrentes.
...
- Causa da correção: o botão lia apenas `POST /recarregar-operacional` (cache local), sem disparar a extração HTTP Transferegov, sem progresso e sem reaproveitar lógica do CLI. Usuário não tinha como atualizar pelo navegador, o que dava falsa impressão de "atualizou" sem consultar a fonte.
- Arquivos criados:
  - `backend/services/profor-2022/profor-pad-atualizacao-transferegov-orquestrador-service.js`: orquestra (1) extração HTTP convênio a convênio reaproveitando `extrairPadTransferegov`, (2) montagem + validação do cache via `profor-pad-transferegov-cache-service`, (3) salvar atomicamente, (4) `carregarPadsOperacional` (recarga). Emite `onProgress({etapa, fase, indice, total, numeroConvenio, uf, mensagem, status, itensExtraidos})`. Aborta antes de salvar se algum convênio falhou tecnicamente (`cache_invalido_ou_inapto`).
  - `backend/services/profor-2022/profor-pad-atualizacao-transferegov-job-service.js`: gerenciador singleton de job em memória local. Garante exclusividade por chave (`global` por padrão): segunda chamada concorrente devolve o job em andamento (`jaEstavaEmAndamento: true`). Mantém até 200 eventos por job, captura erro e expõe `publico(job)` com snapshot serializável.
  - `tests/services/profor-pad-atualizacao-transferegov-orquestrador.test.js`: 11 testes com colaboradores mockados — extração por convênio, gravação só após extração apta, recarga só após cache salvo, progresso completo, bloqueio de concorrência, captura de erro, smoke test garantindo zero alteração em `frontend/data/publicados`.
- Arquivos alterados:
  - `backend/server.js`: dois novos endpoints — `POST /api/profor-2022/pad/atualizar-transferegov` (inicia job; 202 ou 409 se concorrente) e `GET /api/profor-2022/pad/atualizar-transferegov/status/:jobId`.
  - `frontend/js/app.js`: bloco `renderBlocoRecargaOperacionalPadStatusSistema` reescrito com botão principal **"Atualizar PADs no Transferegov"** + botão secundário discreto **"Recarregar somente do cache"** (diagnóstico). Painel de progresso (barra + fase + contador X/15 + convênio atual + UF + mensagem + eventos). Nova função `executarAtualizacaoPadsTransferegovUI` faz POST → polling do GET a cada 1,5 s → renderResultadoRecargaPad ao concluir. Confirmação explícita: "A aplicação irá consultar o Transferegov, atualizar o cache local e reconstruir a visão operacional. Essa operação pode demorar alguns minutos."
  - `scripts/validar-syntax.js`: inclui os 3 arquivos novos.
- Novo fluxo real: clique → confirma → POST inicia job → server roda em background extraindo via `extrairPadTransferegov` os 15 convênios em sequência (sem fallback Playwright) → cada convênio emite evento → após todos OK, salva `backend/data/cache/profor-2022-pad-transferegov-cache.json` (escrita atômica) → chama `carregarPadsOperacional` → emite `resumo` com `resultadoRecarga`. Frontend faz polling a cada 1,5 s e atualiza barra/contador/mensagem/eventos em tempo real. Ao concluir, chama `renderResultadoRecargaPad` (mesma renderização do botão secundário).
- Endpoint/job/progresso: job em memória local (não persiste em banco). `POST` retorna 202 com `jobId` (ou 409 se já houver job em andamento com mesma chave). `GET /status/:jobId` retorna `{jobId, status: em_andamento|concluido|erro, fase, indiceAtual, totalConvenios, convenioAtual, ufAtual, mensagemAtual, eventos[], resultadoRecarga, resumo, erro}`. Estado se limpa quando o job conclui (libera a chave para nova execução).
- Validação real executada: parei servidor antigo (PID 16828, sem novos endpoints) e iniciei nova instância (PID 1800). Chamada real `POST /api/profor-2022/pad/atualizar-transferegov` → job `atualizacao-pad-1779906086710-54e9aa5c6f2c` rodou em ~3 min (180s para 15 convênios via HTTP), 15/15 sucesso, 525 itens, 0 bloqueios técnicos. Cache regravado (hashGlobal `fb95ef8c…`). `resultadoRecarga`: `sucesso=true`, `origem=cache_transferegov`, `arquivosLidos=15`, `totalConvenios=15`, `itensProcessados=525`, `totalAlertas=0`, `totalImpedimentos=0`, `totalPendenciasRevisao=0` (saneamento da sessão anterior persistiu), `aptoParaPublicacao=false`. `GET /status` retornou 409 quando chamado em paralelo, validando bloqueio de concorrência.
- Validações executadas: `git diff --check` (apenas warnings CRLF), `node --check` em `orquestrador-service`, `job-service`, `backend/server.js`, `frontend/js/app.js`. `node --test` em 7 suites relevantes (carregador-operacional, recarga-cache-transferegov, origem-reconstrucao, saneamento-decisoes-antigas, atualizacao-transferegov-orquestrador, revisoes-plano, revisoes-plano-decisoes) → **101/101 pass**. `node scripts/validar-syntax.js` → **111 arquivos OK**.
- Preservações: sem alterar Excel, Playwright, `frontend/data/publicados`, `.env`, `package.json`/`package-lock.json`, DETRU, rendimentos, publicação, banco SQLite por SQL direto, matching ou recarga ou revisão; toda a lógica de extração/montagem/validação/salvamento/recarga é **reuso** dos serviços já testados (`profor-pad-transferegov-extracao-service`, `profor-pad-transferegov-cache-service`, `profor-pad-carregador-operacional-service`). Sem duplicação. Sem novas dependências.
- Arquivos locais fora do commit: `backend/data/cache/profor-2022-pad-transferegov-cache.json` (regravado pela validação real), `backend/data/relatorios/profor-2022-pad-recarga-operacional-v2.json/.md` (regravados pela recarga).
- Risco: baixo. (a) Lógica de extração 100% reaproveitada do script CLI já em produção; (b) cache só é gravado se extração 100% apta (test 3 cobre); (c) bloqueio de concorrência impede dois jobs simultâneos sobrescrevendo cache; (d) botão secundário "Recarregar somente do cache" preserva o caminho diagnóstico anterior; (e) erro no orquestrador é capturado e exposto via `status=erro` com `mensagem`, sem deixar o frontend sem feedback.
- Rollback: `git revert <SHA>` deste commit remove os 2 endpoints, o botão principal e os arquivos novos. O botão secundário "Recarregar somente do cache" voltaria a ser o botão principal anterior (já que o evento ficaria sem o handler novo). O script CLI continua funcional como caminho alternativo. Cache local e relatórios regravados pela validação real permanecem no estado pós-atualização (não revertidos pelo git revert, são intencionalmente fora do commit).
...
### Complemento - edição operacional dos Parâmetros Mínimos e indicador de institucionalização

- Problema encontrado: a alteração de status de cada parâmetro dependia de um botão apenas com ícone, o editor qualitativo oferecia somente duas situações e a ação de salvamento não permanecia evidente; adicionalmente, os rótulos já traduzidos `Pendente` e `Em conformidade` eram normalizados uma segunda vez e podiam aparecer incorretamente como `Não informado`.
- Caso reproduzido: na ficha de Sergipe, `Ato normativo específico` está atualmente registrado como `Pendente`; a interface anterior não deixava claro como atualizar esse item nem que as pendências são derivadas do status salvo.
- Correção funcional: cada item passou a ter ação textual `Alterar status` ou `Alterar quantidade`, editor incorporado ao checklist, cinco estados auditáveis (`Em conformidade`, `Pendente`, `Parcial`, `Validar` e `Não informado`), indicador de alteração não salva, botão contextual `Salvar agora`, barra de ações destacada e confirmação protegida por senha com resumo objetivo da transição.
- Segurança preservada: a página pública continua somente leitura e agora explica que a edição exige a aplicação local autenticada; a escrita real continua usando `POST /api/parametros-minimos/salvar`, o histórico existente e a senha operacional. Nenhum status, pendência, JSON publicado ou dado do Supabase foi alterado nesta tarefa.
- Parâmetros Mínimos: a visão nacional de institucionalização foi posicionada na própria página de diagnóstico, e não no Dashboard financeiro. A contagem é derivada do parâmetro `atoNormativoEspecifico`; o estado atual da fonte publicada resulta em `20` unidades institucionalizadas de `28` unidades diagnosticadas, com `8` pendentes e cobertura de `71%`. A ação `Ver unidades` abre lista pesquisável e filtrável e permite ir diretamente para a ficha da unidade.
- Revisão visual: o checklist passou de cinco colunas estreitas para duas colunas em desktop e uma no celular; botões receberam rótulos e alvos adequados; editores, estados pendentes, modal e painel nacional usam os tokens do tema escuro, com contraste, hierarquia e linguagem institucional mais claros. O cabeçalho deixou de expor o nome interno da planilha e passou a informar fonte, universo diagnosticado e acompanhamento ONASP.
- Arquivos alterados: `frontend/js/app.js`; `frontend/css/app.css`; `tests/services/parametros-minimos-ui.test.js`; e este diário.
- Validação automatizada: `npm run validar:json`; `npm run validar:syntax` (`110` arquivos); `npm run validar:services` (`550` testes, `530` aprovados, `20` ignorados e `0` falhas); e `git diff --check`.
...
## 31/08/2026 - Investigação documental dos itens novos do PAD do Convênio 937782/2022 (AC)

- Escopo solicitado: identificar, com base no Processo SEI 08016.024899/2022-11 e nos dados atuais da aplicação, a destinação dos itens novos ainda não classificados entre Ouvidoria, Corregedoria e Escola Penal, admitindo rateio apenas quando documentalmente sustentado.
- Estado inicial: inspeção somente leitura; nenhum registro do banco, JSON publicado, código ou classificação foi alterado nesta etapa.
- Fontes em análise: arquivo `SEI_08016.024899_2022_11.zip`, com 266 documentos ordenados cronologicamente, leitura prioritária dos documentos mais recentes para os mais antigos; aplicação no repositório Git interno; e projeto Supabase de homologação.
- Critério de decisão: não inferir destinação apenas pelo nome genérico do item. A classificação ou o rateio exigirá vínculo verificável com plano de trabalho, alteração de PAD, projeto básico, nota técnica, ofício ou relatório de execução do Convênio 937782/2022.
- Preservações: documentos SEI brutos não serão copiados para a memória do repositório; credenciais e dados sensíveis não serão expostos nem persistidos.

- Fundamento documental: o Ofício nº 2161/2026/SEJURI/GABS (SEI 36273570) informa que os R$ 15.000,00 de rendimentos se destinam exclusivamente a complementar a aquisição do `Item 005 - Automóvel zero quilômetro`. A Planilha de Alteração do Plano de Trabalho (SEI 36273775) inclui o `Item 023 - Rendimentos de aplicação para complementar o valor do veículo`, natureza 44905299, quantidade 1 e total R$ 15.000,00. A Nota Técnica nº 26/2026/DIAGPI/DIRPP/SENAPPEN/MJ (SEI 36273793) reproduz essa vinculação e aprova o ajuste do PAD.
- Identificação da área: o Projeto Básico atualizado (SEI 33967587), na tabela de recursos por natureza, identifica expressamente o automóvel zero quilômetro de R$ 97.456,67 como `Automóvel Escola Penal`. O item 005 também já possui rateio operacional ativo de 100% para `ESCOLA_PENAL`, coerente com essa fonte documental.
- Conferência dos documentos posteriores: as peças SEI 36746042, 36762227, 36762440, 36766192, 36767000 e 36769868 tratam da solicitação de bloqueio da conta bancária por inexecução financeira e não apresentam redestinação do Item 023. O arquivo contém a expedição do pedido de bloqueio, mas não uma confirmação bancária de que o bloqueio tenha sido efetivado.
- Decisão aplicada: o `Item 023 - Rendimentos de aplicação para complementar o valor do veículo`, quantidade 1 e valor previsto de R$ 15.000,00, foi classificado integralmente como `ESCOLA_PENAL`, sem rateio.
- Persistência e auditoria: a alteração foi realizada pelo endpoint nativo de revisão do PAD, criando o item conhecido 1071 e o rateio ativo 1206 no Supabase; o log operacional 217 registrou a transição `NAO_CLASSIFICADO` para `ESCOLA_PENAL`.
- Validação: a API local retornou o item como `ITEM_PAD`, área `ESCOLA_PENAL` e status `RATEIO_MEMORIZADO_APLICADO`, sem pendência relacionada em SC. Os testes focais passaram em 22/22, `npm run validar:json` foi aprovado e `git diff --check` não apontou erro.
- Efeitos locais: a regeneração operacional atualizou os mesmos relatórios `backend/data/relatorios/profor-2022-pad-recarga-operacional-v2.json` e `.md`, preservando as alterações anteriores de AC; o relatório geral passou a indicar um item novo ainda pendente, não relacionado a SC. Não houve publicação estática, commit ou push.
- Higiene documental: a extração temporária usada para leitura, planilha e renderização foi removida após a validação; o ZIP original permaneceu intacto em `Downloads`.
- Inconsistências documentais não bloqueantes: a planilha e a Nota Técnica nº 26 exibem, para o Item 001, quantidade 2 e valor unitário igual ao valor total de R$ 280.433,34, apesar do valor unitário anterior de R$ 140.216,67 e do status `Manter`; além disso, os Despachos SEI 36762227 e 36762440 mantêm no assunto/interessado referências a outro convênio e ao Distrito Federal, embora o corpo trate do Convênio 937783/2022 de SC. Nenhuma dessas inconsistências altera o vínculo expresso do Item 023 com o Item 005 da Escola Penal.
- Risco administrativo: a classificação setorial está documentalmente sustentada, mas a execução financeira do convênio permanece sujeita às providências de bloqueio solicitadas em agosto de 2026; classificar o item não autoriza movimentação financeira.
- Rollback: reclassificar o Item 023 pelo mesmo fluxo de Revisões PAD, preservando o histórico e desativando o rateio vigente; não excluir diretamente os registros.
- Fundamento documental: o Ofício nº 9912/2026/IAPEN (SEI 36124777) identifica expressamente como `Supervisor (Ouvidoria)` a proposta de 15 meses a R$ 8.050,00, total de R$ 120.750,00, e como `Auxiliar Administrativo (Ouvidoria)` a proposta de 15 meses a R$ 4.730,00, total de R$ 70.950,00. A Planilha de Ajustes (SEI 36124743) repete `(Ouvidoria)` nos dois itens, e a Nota Técnica nº 24/2026/DIAGPI/DIRPP/SENAPPEN/MJ (SEI 36125111) aprova o ajuste do PAD conforme solicitado.
- Conferência de documentos posteriores: o 7º RSE (SEI 36338739), o Despacho nº 328/2026 (SEI 36390593) e a Nota Técnica nº 30/2026 (SEI 36478315) vinculam a contratação da equipe técnica ao Projeto Ouvidoria Itinerante e não apresentam redestinação posterior para Corregedoria ou Escola Penal.
- Decisão aplicada: ambos os itens foram classificados integralmente como `OUVIDORIA`, sem rateio: Auxiliar Administrativo, quantidade 15 e valor previsto de R$ 70.950,00; Supervisor, quantidade 15 e valor previsto de R$ 120.750,00.
- Persistência e auditoria: a alteração foi realizada pelo endpoint nativo de revisão do PAD, criando os itens conhecidos 1069 e 1070 e os rateios ativos 1204 e 1205 no Supabase; os logs operacionais 211 e 214 registraram a transição `NAO_CLASSIFICADO` para `OUVIDORIA`.
- Validação: a API local retornou os dois itens como `ITEM_PAD`, área `OUVIDORIA` e status `RATEIO_MEMORIZADO_APLICADO`; não restou pendência desses itens em AC. Os testes focais de revisão do PAD passaram em 22/22, `npm run validar:json` foi aprovado e `git diff --check` não apontou erro.
- Divergência preservada para análise própria: o PAD operacional atual registra o código de natureza `33903799`, enquanto a situação proposta aprovada na Planilha de Ajustes e na Nota Técnica nº 24/2026 registra `33903400`. Como o pedido tratava apenas da área de destinação, o código de natureza não foi alterado.
- Efeitos locais: a regeneração operacional atualizou apenas os relatórios `backend/data/relatorios/profor-2022-pad-recarga-operacional-v2.json` e `.md`, além deste diário. Não houve publicação estática, commit ou push.
- Higiene documental: a extração temporária usada para leitura e renderização foi removida após a validação; o ZIP original permaneceu intacto em `Downloads`.
- Risco de regressão: baixo para a classificação de área, pois há dupla evidência nominal e aprovação técnica posterior; permanece risco administrativo separado quanto à divergência do código de natureza, que deve ser saneada apenas após conferência específica do PAD vigente no Transferegov.
- Rollback: reclassificar os dois itens pelo mesmo fluxo de Revisões PAD, o que desativa os rateios ativos e preserva a trilha histórica; não executar exclusão direta dos registros.

## 31/08/2026 - Investigação documental do item novo do PAD do Convênio 937783/2022 (SC)

- Escopo solicitado: identificar, com base no Processo SEI 08016.025010/2022-12 e nos dados atuais da aplicação, a destinação do novo `Item 023 - Rendimentos de aplicação...`, ainda não classificado, entre Ouvidoria, Corregedoria e Escola Penal, admitindo rateio apenas se houver evidência documental.
- Estado inicial: preservadas as alterações pendentes do Convênio 937782/2022 (AC); nenhuma classificação de SC foi alterada nesta etapa.
- Fontes em análise: arquivo `SEI_08016.025010_2022_12.zip`, com 212 documentos, leitura prioritária dos documentos mais recentes para os mais antigos; Planilha de Alteração do Plano de Trabalho; aplicação local; e projeto Supabase de homologação.
- Critério de decisão: não classificar por semelhança com itens antigos nem pelo objeto geral do convênio. A área ou o rateio deverá decorrer de plano, projeto básico, ofício, nota técnica, relatório de execução ou planilha formalmente vinculada ao Convênio 937783/2022.
- Preservações: documentos SEI brutos não serão copiados para a memória do repositório; credenciais e dados sensíveis não serão expostos nem persistidos.

## 31/08/2026 - Investigação documental do item novo do PAD do Convênio 937818/2022 (PB)

- Escopo solicitado: identificar, com base no Processo SEI 08016.024920/2022-70 e nos dados atuais da aplicação, a destinação do item novo `Utilização do Rendimento de Aplicação no`, no valor de R$ 401,10, entre Ouvidoria, Corregedoria e Escola Penal, com particionamento somente se documentalmente sustentado.
- Estado inicial: preservadas as classificações e alterações locais pendentes dos convênios do AC e de SC; o item de PB constava como `ITEM_NOVO_SEM_RATEIO`, área `NAO_CLASSIFICADO` e única pendência real do conjunto operacional.
- Fontes examinadas: arquivo `SEI_08016.024920_2022_70.zip`, SHA-256 `FCC737D8F234AFBB99AF936B74E4F307018BBD9BAA29DB9FE5EDF1264BFCA01B`, com 187 documentos, lidos prioritariamente do mais recente para o mais antigo; aplicação local; relatório operacional; e projeto Supabase `fomento-onasp-homologacao`.
- Fundamento documental: a Solicitação de Utilização de Rendimento (SEI 36225859) vincula exclusivamente os R$ 401,10 à elevação do valor unitário do `NOTEBOOK Avançado` de R$ 5.398,90 para R$ 5.800,00. A Nota Técnica nº 28/2026/DIAGPI/DIRPP/SENAPPEN/MJ (SEI 36333501), última peça do arquivo, reproduz os valores, manifesta-se pela aprovação e foi aprovada pelo Diretor de Políticas Penitenciárias em 24/07/2026.
- Identificação da área: o Apêndice B do Ofício SAP-OFI-2025/08095 (SEI 32607808) registra o mesmo `NOTEBOOK Avançado`, código 128051 e valor de R$ 5.398,90, com distribuição de `0` unidade para Corregedoria, `0` para Ouvidoria e `1` para EGEPEN. O item conhecido correspondente também possuía rateio operacional ativo de 100% para Escola Penal, confirmando a correspondência sem substituir a prova documental.
- Decisão aplicada: o item `Utilização do Rendimento de Aplicação no`, quantidade 1 e valor previsto de R$ 401,10, foi classificado integralmente como `ESCOLA_PENAL`, sem particionamento.
- Persistência e auditoria: o fluxo nativo de Revisões PAD criou o item conhecido 1072 e o rateio ativo 1207, com 100% da quantidade e do valor em `ESCOLA_PENAL`; o log operacional 220 registrou a transição `NAO_CLASSIFICADO` para `ESCOLA_PENAL`.
- Validação: a API local passou a retornar `ITEM_PAD`, `RATEIO_MEMORIZADO_APLICADO` e nenhuma pendência para PB; o resumo de PB passou a `statusRevisao=OK`, e o relatório geral ficou com 568 linhas reconstruídas, 525 rateios aplicados, zero item novo sem rateio, zero impedimento e zero alerta. Os testes focais passaram em 22/22, `npm run validar:json` foi aprovado e `git diff --check` não apontou erro.
- Efeitos locais: foram atualizados apenas os relatórios operacionais `backend/data/relatorios/profor-2022-pad-recarga-operacional-v2.json` e `.md`, além deste diário; as alterações anteriores de AC e SC foram preservadas. Não houve publicação estática, commit ou push.
- Inconsistências documentais não bloqueantes: a Solicitação SEI 36225859 menciona uma vez o instrumento `937818/2023`, embora o assunto, o processo e a Nota Técnica posterior identifiquem corretamente o Convênio 937818/2022; o plano atualizado de 2025 registra o notebook com natureza 44905235, enquanto a Nota Técnica nº 28/2026 e o PAD operacional atual usam 44905299. Como o escopo desta análise é exclusivamente a área, nenhum código de natureza foi alterado.
...
## 2026-09-10 — RO: atualização para o PAD proposto (Ofício nº 32555/2026/SEJUS-GECON)

- Branch: main; base 7fa34b4.
- Escopo: atualizar o convênio RO (937917) na aplicação para o **PAD proposto** pela alteração solicitada no Ofício nº 32555/2026/SEJUS-GECON (SEI 0033.095554/2022-13), por decisão expressa do usuário.
- Contexto: o PAD obtido no Transferegov em 10/09/2026 corresponde ao PAD **proposto** (28 itens consolidados), enquanto a aplicação continha o PAD **aprovado** (37 linhas). Os totais são idênticos — R$ 234.072,01 previsto e R$ 13.450,00 executado — tratando-se de remanejamento, não de alteração de valor global. A divergência de descrições, quantidades e valores unitários entre os dois conjuntos decorre dessa diferença de estágio.
- Correção: o `planoAplicacao` do RO foi substituído por **33 linhas** derivadas dos 28 itens do PAD proposto; os agregados por área foram recalculados; o campo `origemDados` do registro foi marcado como `PAD PROPOSTO - Oficio n. 32555/2026/SEJUS-GECON (em tramitacao)`.
- Regra de construção (explícita e auditável): descrição, quantidade e valores vindos do **PAD proposto**; área vinda da **memória de classificação** (PAD aprovado + tabela comparativa do Ofício); itens com rateio divididos em uma linha por área, com a quantidade do PAD aprovado e o valor unitário do PAD proposto, de modo que a soma preserve o total do item; item de saldo remanescente classificado como `N/A`, conforme regra de classificação definida pelo usuário.
- Casos resolvidos pelo Ofício: `Nobreak 1.500Va` (20 un) = Ouvidoria 12 + Corregedoria 8 (agrupamento dos itens 8 e 35; item 9, da Escola Penal, excluído); `Monitor de 23 polegadas` (20) = Corregedoria 8 + Ouvidoria 12; `Computador de Mesa (Desktop)` (14) = Ouvidoria 12 + Corregedoria 2; `Cadeira em Longarina 3 lugares` (7) = Corregedoria 3 + Escola Penal 4; `Central de ar-condicionado de 12.000 BTU/h` (8) = Escola Penal 3 + Corregedoria 5. `Saldo Remanescente` (1) = N/A.
- Resultado por área: Corregedoria R$ 69.567,76 (11 linhas); Ouvidoria R$ 68.192,44 (8 linhas); Escola Penal R$ 90.866,42 (13 linhas); N/A R$ 5.445,39 (1 linha). Total: 33 linhas, R$ 234.072,01.
- Arquivos: `frontend/data/publicados/aplicacao.json`; `frontend/data/publicados/dashboard-geral.json`; este diário.
- Validações: ambos os JSONs reparseados com sucesso; a soma das linhas do RO confere com os agregados do próprio registro, tanto em previsto quanto em executado; o total confere com o cabeçalho do PAD proposto; as demais 14 UFs foram conferidas e permanecem intactas (AC com 49 linhas e R$ 396.423,71). Backup prévio dos dois JSONs em `C:\Users\marce\dsh-browser\backup-fomento-20260910-124839`.
- Limite: os **JSONs publicados foram alterados diretamente**, sem execução de `npm run publicar:dados`. A regeneração a partir da origem/banco não foi executada e poderia reverter esta alteração. Não houve commit, push, publicação nem reinício de backend. **O site publicado (GitHub Pages) não reflete esta mudança** até que haja commit e push.
- Preservação: nenhum outro convênio foi alterado; nenhum arquivo de memória, planilha, código ou teste foi modificado; nenhuma dependência foi adicionada; nenhuma credencial coletada.
- Risco: **publica um PAD ainda em tramitação**, cujos itens constam como "Aguardando aprovação" no Ofício, contrariando o princípio da aplicação de não publicar alteração pendente. A decisão foi expressa do usuário. Caso a alteração não seja aprovada pela SENAPPEN, o RO deverá ser revertido ao PAD aprovado — que é o estado anterior registrado no backup.
- Rollback: restaurar os dois JSONs a partir do backup, ou `git checkout -- frontend/data/publicados/aplicacao.json frontend/data/publicados/dashboard-geral.json`.

## 2026-09-10 — RO: tentativa de carga pelo caminho oficial — bloqueada por indisponibilidade do banco

- Branch: main; base 7fa34b4 (mesma sessão do registro anterior).
- Escopo: migrar a atualização do RO do JSON publicado para a via oficial — carga do PAD na pasta operacional, recarga no banco, rateio pelos serviços e republicação.
- Executado: o PAD do RO em `Planilhas/profor-2022/instrumentos` foi **substituído**. Saiu `RelatorioItensDespesasPAD_2026_05_20-13_543287213509088727749.xls` (37 itens — PAD aprovado) e entrou `RelatorioItensDespesasPAD_2026_09_10-12_031250006292349625513.xls` (28 itens — PAD proposto). A pasta permanece com 15 arquivos. Cópia do arquivo removido em `C:\Users\marce\dsh-browser\backup-fomento-20260910-124839\PAD-RO-APROVADO-20-05-2026-RelatorioItensDespesasPAD_2026_05_20-13_543287213509088727749.xls`.
- **Bloqueio:** o backend é Postgres-only — `preparar-banco.js` recusa o boot sem `DATABASE_URL`, com mensagem explícita de que não há fallback para SQLite — e o banco Supabase está **inacessível**. Evidência: `logs/launcher-server.err.log` registra `error: (EAUTHQUERY) authentication query failed: connection to database not available`, e `logs/launcher-supabase.log` registra tentativa de início em 10/09/2026 12:51 que falhou após 13 segundos. Não há `DATABASE_URL` no `.env`; não há Postgres local (porta 5432 inativa). O `better-sqlite3` instalado está compilado para outro NODE_MODULE_VERSION (147, contra 141 do Node atual), mas isso é irrelevante: o SQLite é legado e a aplicação não o utiliza mais no boot.
- **Não executado:** carga do PAD no banco, registro do rateio/classificação dos 28 itens do RO e republicação pela via oficial. **Nada foi escrito no banco de dados.**
- **Pendência para retomada:** (1) reativar o projeto Supabase; (2) iniciar pelo acionador `scripts/windows/iniciar-fomento-supabase.ps1`, que solicita a senha do banco interativamente; (3) rodar a recarga operacional com o PAD novo; (4) registrar o rateio do RO pelos serviços, no padrão do script `sanear-classificacao-pad-al-937221-profor-2022.js`, nunca por SQL direto; (5) republicar; (6) revalidar a aplicação contra o XLS e contra as áreas apuradas no Ofício nº 32555/2026.
- **Estado intermediário conhecido — divergência entre camadas:** pasta operacional com o PAD **proposto**; JSONs publicados com o PAD **proposto**; banco de dados ainda com o PAD **aprovado** (37 itens e 74 rateios em 3 áreas). As três camadas convergem quando a recarga operacional for executada com o banco disponível.
- **Preservação:** nenhum arquivo de código, teste, memória ou planilha foi alterado além do PAD do RO; nenhum commit, push ou reinício de backend; nenhuma credencial coletada, exibida ou armazenada.
- Rollback: além do rollback dos JSONs já registrado acima, restaurar o arquivo `..._543287213509088727749.xls` a partir do backup para `Planilhas/profor-2022/instrumentos`.
