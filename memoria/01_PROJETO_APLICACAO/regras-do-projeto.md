...
A memória versionada do repositório deve priorizar fichamentos técnicos em Markdown, sintéticos, rastreáveis e úteis para manutenção, análise institucional, elaboração de minutas, revisão técnica e uso por IA.

O repositório não mantém estrutura própria de documentos brutos. PDFs, DOCX, XLSX, imagens, anexos SEI, bases originais, planilhas completas, logs e arquivos sensíveis não devem ser copiados para a pasta `memoria/` como fonte primária de trabalho.

Os documentos originais devem permanecer na origem externa ou institucional correspondente. O fichamento técnico deve identificar a fonte original de forma suficiente para rastreabilidade, sem reproduzir integralmente o documento bruto.
...
- referência ou localização institucional do original, quando puder ser indicada;
- escopo de leitura;
- partes analisadas;
...
O fichamento técnico não substitui o documento original. Para citação formal, fundamento jurídico, artigo, inciso, transcrição literal, dado sensível, informação orçamentária ou conferência de validade, o documento original deve ser consultado.
