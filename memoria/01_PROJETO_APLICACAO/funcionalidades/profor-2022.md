...
A visão geral da home ("Fomento para Ouvidoria") e o rodapé deixam de exibir o texto estático "Atualizado em abril de 2026" e passam a exibir a data/hora dinâmica calculada como `max(últimaAtualizaçãoDETRU, últimaConsultaRendimentosTransferegov)`. O endpoint `GET /api/profor-2022/atualizacao/status` ganhou o campo `ultimaAtualizacaoDados = { dataHora, fonte, fontesConsideradas: { detru, rendimentos } }`. O frontend formata a data em padrão brasileiro `dd/mm/aaaa às HH:MM` em fuso local do navegador. Em modo estático/GitHub Pages, o endpoint não é chamado e o texto exibido é "Atualização não registrada" — sem inventar horário. A nota "Os dados foram atualizados até abril de 2026 (...) janela de submissão dos relatórios" segue inalterada por se tratar de aviso de governança específico do FAF.
